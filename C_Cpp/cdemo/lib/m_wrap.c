...
Inet_pton(int family, const char *strptr, void *addrptr)
{
	...
	if ( (n = sock_inet_pton(family, strptr, addrptr)) < 0)
		err_sys("inet_pton error for %s( < 0)", strptr);	/* errno set */
	else if (n == 0)
		err_quit("inet_pton error for %s( == 0)", strptr);	/* errno not set */
...
{
	if (sock_writen(fd, ptr, nbytes) != nbytes)
...
Readline(int fd, void *ptr, size_t maxlen)
{
	...
	if ( (n = sock_readline(fd, ptr, maxlen)) < 0)
